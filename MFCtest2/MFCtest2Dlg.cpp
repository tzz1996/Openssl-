...
void CMFCtest2Dlg::OnBnClickedOk()
{
	...
	if (radio5 == 1) {
		if (vedio_encrypt.EDncryptFile(input_string, output_string, 1))
			MessageBox("文件加密完成", "提示", MB_ICONINFORMATION | MB_OK);
		radio5 = 0;
	}
	if (radio6 == 1) {
		if (vedio_encrypt.EDncryptFile(input_string, output_string, 0))
			MessageBox("文件解密完成", "提示", MB_ICONINFORMATION | MB_OK);
		radio6 = 0;
	}
}


void CMFCtest2Dlg::OnEnChangeEdit1()     //输入文件文本框
{
...
void CMFCtest2Dlg::OnEnChangeEdit3()     //输出文件文本框
{
	// TODO:  如果该控件是 RICHEDIT 控件，它将不
	// 发送此通知，除非重写 CDialogEx::OnInitDialog()
	// 函数并调用 CRichEditCtrl().SetEventMask()，
...
void CMFCtest2Dlg::OnBnClickedRadio5()  //加密按钮
{
	// TODO: 在此添加控件通知处理程序代码
...
void CMFCtest2Dlg::OnBnClickedRadio6()  //解密按钮
{
	// TODO: 在此添加控件通知处理程序代码
